...
locals {
  ...
  config_tables = [
    for table in local.tpl_tables : merge(
      {
        file_path      = table
        file_stem_path = trimsuffix(table, ".yaml")
        file_stem      = basename(trimsuffix(table, ".yaml"))
      },
      yamldecode(
        templatefile(
  ...
  tables = {
    for table in local.config_tables : "${table.file_stem_path}" => {
      reference = join(".", [
        local.dataset_map[lookup(table, "dataset", lookup(table, "dataset_id", null))].project,
        local.dataset_map[lookup(table, "dataset", lookup(table, "dataset_id", null))].dataset_id,
        "${table.table_id}_v${table.version}"
      ])
      dataset_id         = local.dataset_map[lookup(table, "dataset", lookup(table, "dataset_id", null))].dataset_id
      project            = local.dataset_map[lookup(table, "dataset", lookup(table, "dataset_id", null))].project
      ...
      schema_string = format("(%s)", join(", ", [
        for column in lookup(table, "schema", []) : join(" ", [
          column.name,
          lookup(column, "mode", "NULLABLE") == "REPEATED" ? "ARRAY<${column.type}>" : column.type,
          lookup(column, "mode", "NULLABLE") == "REQUIRED" ? "NOT NULL" : ""
        ])
      ]))

...
locals {
  ...
  schedulers_main = {
    for file_path in local.tpl_schedulers_main :
    trimsuffix(file_path, ".yaml") => merge(
      ...
      yamldecode(
        templatefile(
          "${local.configuration_folder}/triggers/schedulers/${file_path}",
          merge(
            local.template_vars,
            {
...
resource "google_cloud_scheduler_job" "schedulers" {
  ...
  name             = "${local.app_name_short}-sch-${each.value.file_stem}-${local.region_id}-${local.project_env}"

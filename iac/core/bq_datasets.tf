...
locals {
  ...
  config_datasets = [
    for dataset in local.tpl_datasets : {
      file_path      = dataset
      file_stem_path = trimsuffix(dataset, ".yaml")
      file_stem      = basename(trimsuffix(dataset, ".yaml"))
      config = yamldecode(
        templatefile(
  ...
  internal_config_datasets = [
    for dataset in local.config_datasets : dataset
    if !(lookup(dataset.config, "is_external", false))
  ]
  external_config_datasets = [
    for dataset in local.config_datasets : dataset
    if(lookup(dataset.config, "is_external", false))
  ]

  # Read config from yaml file and fill missing values with default one
  datasets_internal_output = [
    for dataset_info in local.internal_config_datasets : {
      project                = lookup(dataset_info.config, "project", local.project)
      dataset_file_stem_path = dataset_info.file_stem_path
      dataset_id = join("", [
        "${local.app_name_short}_ds_${lookup(dataset_info.config, "confidentiality", "c3")}",
        "_${dataset_info.file_stem}",
      ...
      default_table_expiration_ms = length(regexall("(tmp)$", dataset_info.file_stem_path)) == 0 ? null : lookup(dataset_info.config, "default_table_expiration_ms", "432000000")
  ...
  datasets_external_output = [
    for dataset_info in local.external_config_datasets : {
      project                = dataset_info.config.project
      dataset_file_stem_path = dataset_info.file_stem_path
      dataset_id             = dataset_info.config.dataset_id
      is_external            = true
      location               = lookup(dataset_info.config, "location", null)
      confidentiality        = lookup(dataset_info.config, "confidentiality", null)
      permissions            = lookup(dataset_info.config, "permissions", [])
    }
  ]
  datasets_output = concat(local.datasets_internal_output, local.datasets_external_output)

  #convert the values into map format
  dataset_map = ({
    for dataset in local.datasets_output :
    dataset.dataset_file_stem_path => merge(
      dataset,
      { reference = "${dataset.project}.${dataset.dataset_id}" }
    )
  })

  internal_datasets = { for k, v in local.dataset_map : k => v if v.is_external == false }
  external_datasets = { for k, v in local.dataset_map : k => v if v.is_external == true }


  # Get permissions from dataset config file
  permissions_output = [
    for dataset_output in local.internal_datasets : {
...
resource "google_bigquery_dataset" "datasets" {
  for_each      = local.internal_datasets
